...
// Root — confirm server is alive
app.get('/', function(req, res) {
  res.json({
    service: 'Lahara Intelligence Backend',
    status:  'running',
    routes: [
      'GET  /health',
      'GET  /api/markets',
...
app.get('/api/markets', async function(req, res) {
  ...
  try {
    ...
    res.json({ source: 'live', data: data });
  } catch (e) {
...
app.get('/api/news', async function(req, res) {
  ...
  res.json({ source: 'live', articles: articles });
});
...
app.post('/api/enrich', async function(req, res) {
  ...
  const q_yes     = body.q_yes || 0.5;
  if (!question) return res.status(400).json({ error: 'question required' });
  const cacheKey = 'enrich:' + String(market_id || question).slice(0, 60);
  const cached = getCache(cacheKey);
  if (cached) return res.json(Object.assign({ source: 'cache' }, cached));
  try {
    const result = await enrichMarket({ question: question, market_id: market_id, q_yes: q_yes });
    setCache(cacheKey, result);
    res.json(Object.assign({ source: 'live' }, result));
  } catch (e) {
...
// Batch enrich
app.post('/api/enrich-batch', async function(req, res) {
  const body    = req.body || {};
  const markets = Array.isArray(body.markets) ? body.markets.slice(0, 25) : [];
  ...
  while (i < markets.length) {
    ...
    for (var j = 0; j < enriched.length; j++) {
      results.push(enriched[j]);
    }
    i = i + BATCH_SIZE;
    if (i < markets.length) await sleep(300);
  }

  res.json({ results: results });
});

// Catch-all 404 — helpful instead of silent
app.use(function(req, res) {
  res.status(404).json({
    error:   'Route not found',
    tried:   req.method + ' ' + req.path,
    available: ['GET /', 'GET /health', 'GET /api/markets', 'GET /api/news', 'POST /api/enrich', 'POST /api/enrich-batch']
  });
});
...
async function enrichMarket(opts) {
  ...
  const market_id = opts.market_id;
  const q_yes     = Number(opts.q_yes) || 0.5;
  ...
  const allArticles = deduplicate(parts[0].concat(parts[1]).concat(parts[2]));
  const newsCount   = allArticles.length;
  ...
  const scored  = allArticles.map(function(a) { return scoreArticle(a, question); });
  const signals = aggregateSignals(scored, q_yes);
...
async function fetchRSSFeeds(keywords) {
  ...
  for (var i = 0; i < feeds.length; i++) {
    try {
      ...
      const res        = await fetch(feeds[i], { signal: controller.signal });
      clearTimeout(timer);
...
async function fetchAllNews(keywords) {
  ...
  return deduplicate(parts[0].concat(parts[1]).concat(parts[2]));
}
...
function extractKeywords(question) {
  const stopWords = new Set([
    ...
    'above','below','over','under','per','percent','billion','million',
    'trillion','end','close','start','begin','result','announce',
    'release','reach','exceed','fall','rise','drop','pass'
  ]);
  ...
  const entities = [];
  const capRx    = /([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)*)/g;
  var m;
  ...
  const combined = entities.slice(0, 3).concat(topWords);
  const seen     = new Set();
  const unique   = [];
...
const POS_WORDS = new Set([
  ...
  'beat','beats','exceed','exceeds','exceeded','outperform','strong',
  'approve','approved','pass','passes','passed','win','wins','won','victory',
  'support','backed','likely','probable','confidence','optimism','optimistic',
  'record','high','breakthrough','agreement','deal','signed','confirm',
  'confirmed','increase','expand'
]);
...
const NEG_WORDS = new Set([
  ...
  'delay','delayed','dispute','crisis','cut','cuts','ban','banned',
  'sanction','uncertainty','volatile','volatility','weak','weaker','halt'
]);
...
function sentimentScore(text) {
  ...
  const negRx = /\b(not|no|never|don't|doesn't|didn't|won't|can't)\s+(\w+)/gi;
  var nm;
  while ((nm = negRx.exec(text)) !== null) {
    const w = nm[2].toLowerCase();
...
function scoreArticle(article, question) {
  const qWords   = new Set(extractKeywords(question).map(function(w) { return w.toLowerCase(); }));
  const fullText = (article.title + ' ' + (article.body || '')).toLowerCase();
  var hits = 0;
  qWords.forEach(function(w) { if (fullText.indexOf(w) !== -1) hits++; });
  const relevance = qWords.size > 0 ? hits / qWords.size : 0;
  const sentiment = sentimentScore(fullText);
  const direction = sentiment > 0.1 ? 'BULLISH' : sentiment < -0.1 ? 'BEARISH' : 'NEUTRAL';
...
function aggregateSignals(scored, q_yes) {
  const weighted = scored.filter(function(a) { return a.relevance > 0.1; });

  if (weighted.length === 0) {
    return { predP: q_yes, predConf: 0.42, bdisFinancial: 48, sentiment: 0, keySignals: [] };
  }

  const totalWeight  = weighted.reduce(function(s, a) { return s + a.relevance; }, 0);
  const avgSentiment = weighted.reduce(function(s, a) { return s + a.sentiment * a.relevance; }, 0) / totalWeight;
  const volumeConf   = Math.min(0.25, weighted.length * 0.025);
  const predP        = Math.max(0.05, Math.min(0.95, q_yes + avgSentiment * 0.12));
  const sentConf     = Math.min(0.20, Math.abs(avgSentiment) * 0.25);
  const predConf     = Math.min(0.92, 0.50 + volumeConf + sentConf);
  const avgRelevance = totalWeight / weighted.length;
  const consensus    = (avgSentiment > 0.15 || avgSentiment < -0.15) ? 15 : 5;
  const bdisFinancial = Math.min(95, Math.max(30,
    40 + avgRelevance * 30 + consensus + weighted.length * 1.5
  ));

  const keySignals = weighted
    .slice()
...
app.listen(PORT, '0.0.0.0', function() {
  console.log('');
  console.log('  Lahara Backend started');
  console.log('  PORT    : ' + PORT);
  console.log('  NewsAPI : ' + (NEWS_API_KEY ? 'configured' : 'NOT SET'));
  console.log('  CORS    : ' + ALLOWED_ORIGIN);
  console.log('  Routes  : GET /  GET /health  GET /api/markets  GET /api/news  POST /api/enrich  POST /api/enrich-batch');
  console.log('');
});
